...
// Use Jikan's Chinese animation (Donghua) — producer country filter not in v4,
// use type=ONA which covers most Chinese animation, or search for "donghua"
$cacheKey = 'latest_cn_page:' . $page;
$cached   = getCache($cacheKey);
if ($cached && is_array($cached)) {
    $items    = $cached['items']    ?? [];
    $lastPage = $cached['lastPage'] ?? 1;
} else {
    // Chinese animation tends to be donghua — search for it
    $animeService = new AnimeService();
    $resp  = $animeService->getByProducer(1, $page, 24, 'start_date', 'desc');
    // Fallback to just ONA/currently airing
    if (empty($resp['data'])) {
        $resp = $animeService->getByType('ona', $page, 24);
    }
    $items = [];
...
$pageTitle    = 'Latest Chinese Anime (Donghua) — ' . $websiteTitle;
$pageDesc     = 'Watch the latest Chinese anime (Donghua) online in HD on ' . $websiteTitle . '.';
$pageCanonical = $websiteUrl . '/latest/chinese' . ($page > 1 ? '?page=' . $page : '');
  ...
  <div class="ak-catalog-wrap">
    ...
    <main class="ak-catalog-main">
      ...
      <nav class="ak-breadcrumb">
        ...
        <span class="sep">/</span>
        <span class="current">Chinese Anime</span>
      </nav>

      <div class="ak-catalog-header">
        <h1 class="ak-catalog-title"><i class="fas fa-dragon"></i> Chinese Anime (Donghua)</h1>
      </div>
    ...
    <aside class="ak-catalog-sidebar">
      <div class="ak-sidebar-box">
        ...
        <ul class="ak-sidebar-links">
          <li><a href="<?=$websiteUrl?>/latest/subbed"><i class="fas fa-closed-captioning"></i> Latest Subbed</a></li>
          ...
          <li><a href="<?=$websiteUrl?>/type?type=ova"><i class="fas fa-compact-disc"></i> OVA</a></li>
          <li><a href="<?=$websiteUrl?>/type?type=ona"><i class="fas fa-play-circle"></i> ONA</a></li>
        </ul>
      </div>
    </aside>
