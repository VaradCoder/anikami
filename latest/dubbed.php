...
// Jikan doesn't have a direct "dubbed" filter; use bypopularity and filter for (Dub) titles
// or show top airing and note that dubbed content is limited
$cacheKey = 'latest_dub_page:' . $page;
$cached   = getCache($cacheKey);
if ($cached && is_array($cached)) {
    $items    = $cached['items']    ?? [];
    $lastPage = $cached['lastPage'] ?? 1;
} else {
    // Pull from popular (larger pool to find dubs) and filter
    $resp = (new SearchService())->searchRaw('%28Dub%29', 24, $page);
    $items = [];
...
$pageCanonical = $websiteUrl . '/latest/dubbed' . ($page > 1 ? '?page=' . $page : '');
  ...
  <div class="ak-catalog-wrap">
    ...
    <main class="ak-catalog-main">
      ...
      <nav class="ak-breadcrumb">
        ...
        <span class="sep">/</span>
        <span class="current">Latest Dubbed</span>
      </nav>
          ...
          <a href="<?=$websiteUrl?>/anime/<?=app_e($aid)?>" class="ak-card-poster-link">
            <div class="ak-card-poster">
              ...
              <div class="ak-card-badges">
                <span class="ak-card-badge ak-badge-dub">DUB</span>
