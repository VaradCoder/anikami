...
if ($cached && is_array($cached)) {
    $popularRows = $cached['items']    ?? [];
    $lastPage    = $cached['lastPage'] ?? 1;
} else {
    [$popularRows, $lastPage] = legacy_json_list_from_jikan('top/anime?filter=bypopularity', $page);
    setCache($cacheKey, ['items' => $popularRows, 'lastPage' => $lastPage], 14400);
}

$pageTitle    = 'Most Popular Anime — ' . $websiteTitle;
$pageDesc     = 'Browse the most popular anime of all time on ' . $websiteTitle . '. Watch HD anime online for free.';
$pageCanonical = $websiteUrl . '/popular' . ($page > 1 ? '?page=' . $page : '');
...
$genreList = [
    ...
    'Supernatural','Thriller','Vampire','Harem'
];
?>
<!DOCTYPE html>
<html lang="en">
  ...
  <div class="ak-catalog-wrap">
    ...
    <main class="ak-catalog-main">
      ...
      <nav class="ak-breadcrumb">
        ...
        <span class="sep">/</span>
        <span class="current">Most Popular</span>
      </nav>

      <div class="ak-catalog-header">
        <h1 class="ak-catalog-title"><i class="fas fa-fire"></i> Most Popular Anime</h1>
        <?php if ($page > 1): ?>
      ...
      <div class="ak-anime-grid">
        <?php foreach ($popularRows as $item):
