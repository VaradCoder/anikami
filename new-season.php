...
$cacheKey = 'new_season_page:' . $page;
$cached   = getCache($cacheKey);
if ($cached && is_array($cached)) {
    $seasonRows = $cached['items']    ?? [];
    $lastPage   = $cached['lastPage'] ?? 1;
} else {
    [$seasonRows, $lastPage] = legacy_json_list_from_jikan('seasons/now', $page);
    setCache($cacheKey, ['items' => $seasonRows, 'lastPage' => $lastPage], 7200);
}

$pageTitle    = 'New Season Anime — ' . $websiteTitle;
$pageDesc     = 'Watch the latest new season anime on ' . $websiteTitle . '. Stream current season anime in HD.';
$pageCanonical = $websiteUrl . '/new-season' . ($page > 1 ? '?page=' . $page : '');
  ...
  <div class="ak-catalog-wrap">
    ...
    <main class="ak-catalog-main">
      ...
      <nav class="ak-breadcrumb">
        ...
        <span class="sep">/</span>
        <span class="current">New Season</span>
      </nav>

      <div class="ak-catalog-header">
        <h1 class="ak-catalog-title"><i class="fas fa-leaf"></i> New Season Anime</h1>
        <?php if ($page > 1): ?>
      ...
      <div class="ak-anime-grid">
        <?php foreach ($seasonRows as $item):
